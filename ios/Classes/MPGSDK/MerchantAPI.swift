...
class MerchantAPI {
    ...
    func createSession(completion: @escaping (Result<GatewayMap>) -> Void) {
        issueRequest(path: "session", method: "POST", completion: completion)
    }
...
//
//        issueRequest(path: "3DSecureId/\(transaction.threeDSecureId!)", method: "PUT", body: payload, completion: completion)
//    }
//
...
//        let loginData = String(format: "%@:%@", config.environment.MPGSMerchantUser, config.environment.MPGSMerchantPassword).data(using: String.Encoding.utf8)!
